...
def build_video_mcq_generation_messages(
    ...
    metadata = {
        ...
        "requested_number_of_embedded_questions": int(num_check_questions),
    ...
    user_text = (
        ...
        "QUESTION TYPES:\n"
        "1. pre_question:\n"
        "- Appears before the video starts at timestamp 0.\n"
        "- Should activate prior knowledge, expose a likely misconception, or create curiosity.\n"
        "- Should be answerable using background knowledge/reasoning, not only by memorising content revealed later.\n"
        "- Must align with the central learning goal of the video.\n\n"
        ...
        "- Use only if the transcript suggests a suitable prediction moment, such as a teacher question or a clear upcoming reveal.\n"
        ...
        "}\n\n"
        "Important:\n"
        f"- Generate exactly 1 pre_question and up to {int(num_check_questions)} embedded_questions.\n"
...
def normalise_mcq_item(item: Dict[str, Any], fallback_id: str, fallback_type: str) -> Dict[str, Any]:
    ...
    item.setdefault("question_type", fallback_type)
...
def generate_video_mcq(
    ...
    use_frame_analysis: bool = Form(True),
    ...
    try:
        ...
        num_check_questions = max(1, min(10, int(num_check_questions)))
        ...
        raw_output = call_video_llm(messages, max_new_tokens=3000)
